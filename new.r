...
# Zamiana NA na 0 w kolumnach liczbowych
...
# Suma wiersza
dane$podatek_sum <- rowSums(dane[, c("podatek_uop", "podatek_dg", "podatek_fin")], na.rm = TRUE)

# Suma kumulacyjna
dane$suma_kumulacyjna <- cumsum(ifelse(is.na(dane$podatek_sum), 0, dane$podatek_sum))
...
dane_3 <- dane %>%
  ...
  # sumy wierszy
  mutate(
    podatek_sum_A = rowSums(across(c(podatek_uop_A, podatek_dg_A, podatek_fin)), na.rm = TRUE),
    podatek_sum_B = rowSums(across(c(podatek_uop_B, podatek_dg_B, podatek_fin)), na.rm = TRUE)
  )



suma_podatek_sum   <- sum(dane_3$podatek_sum, na.rm = TRUE)
suma_podatek_sum_A <- sum(dane_3$podatek_sum_A, na.rm = TRUE)
suma_podatek_sum_B <- sum(dane_3$podatek_sum_B, na.rm = TRUE)

# Przygotowanie danych do wyświetlenia
wyniki <- data.frame(
  Scenariusz = c("Obecny system", "Scenariusz A", "Scenariusz B"),
  Suma_podatków = c(suma_podatek_sum, suma_podatek_sum_A, suma_podatek_sum_B),
  Różnica_względem_obecnego = c(0, 
                                suma_podatek_sum_A - suma_podatek_sum,
                                suma_podatek_sum_B - suma_podatek_sum)
)
...
cat("1. Scenariusz A zmienia wpływy o", 
    round((suma_podatek_sum_A/suma_podatek_sum - 1)*100, 1), "%\n")
cat("2. Scenariusz B zmienia wpływy o", 
    round((suma_podatek_sum_B/suma_podatek_sum - 1)*100, 1), "%\n")
